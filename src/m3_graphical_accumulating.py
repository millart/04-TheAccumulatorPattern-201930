...
def draw_parallel_lines(n, point, length, window):
    ...
    line_1 = rg.Line(point, rg.Point(point.x + length, point.y))
    line_1.attach_to(window)
    window.render()

    for k in range(n):
        line_new = rg.Line(rg.Point(point.x, point.y + (k * 30)) , rg.Point(point.x + length, point.y + (k * 30)))
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        line_new = rg.Line(point, rg.Point(point.x + 100, point.y + ((change_in_y * k) - 100)))
        line_new.attach_to(window)
        window.render()
