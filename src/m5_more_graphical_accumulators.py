...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        square = rg.Square(rg.Point(x + (circle.radius * k), y + (circle.radius * k)), (circle.radius * 2))
...
def run_test_draw_circles_from_rectangle():
    ...
    # Test 1:
    rectangle1 = rg.Rectangle(rg.Point(315, 175), rg.Point(330, 250))
    rectangle1.fill_color = 'red'
    draw_circles_from_rectangle(4, 1, rectangle1, window1)

    # Test 2:
    rectangle2 = rg.Rectangle(rg.Point(550, 200), rg.Point(575, 220))
    draw_circles_from_rectangle(2, 5, rectangle2, window1)
    window1.close_on_mouse_click()
    ...
    # Test 3:
    rectangle3 = rg.Rectangle(rg.Point(275, 200), rg.Point(285, 225))
    rectangle3.fill_color = 'green'
    draw_circles_from_rectangle(7, 10, rectangle3, window2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    window.render()
    left_edge = rectangle.corner_1.x
    left_center = rectangle.get_center().y
    left_radius = (rectangle.corner_2.y - rectangle.corner_1.y) / 2
    top_edge = rectangle.corner_1.y
    top_center = rectangle.get_center().x
    top_radius = (rectangle.corner_2.x - rectangle.corner_1.x) / 2

    for k in range(m):
        circle1 = rg.Circle(rg.Point(left_edge - left_radius - (2 * left_radius * k), left_center), left_radius)
        circle1.fill_color = rectangle.fill_color
        circle1.attach_to(window)
        window.render()

    for k in range(n):
        circle2 = rg.Circle(rg.Point(top_center, top_edge - top_radius - (2 * top_radius * k)), top_radius)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    length_line1_x = rectangle2.get_center().x - rectangle1.get_center().x
    length_line1_y = rectangle2.get_center().y - rectangle1.get_center().y
    if rectangle1.corner_1.x < rectangle1.corner_2.x:
        start_x = rectangle1.corner_1.x
        x = rectangle1.corner_1.x
    else:
        start_x = rectangle1.corner_2.x
        x = rectangle1.corner_2.x
    if rectangle1.corner_1.y > rectangle1.corner_2.y:
        start_y = rectangle1.corner_1.y
        y = rectangle1.corner_1.y
    else:
        start_y = rectangle1.corner_2.y
        y = rectangle1.corner_2.y

    for k in range(n-1):
        line = rg.Line(rg.Point(start_x, start_y), rg.Point(start_x + length_line1_x, start_y + length_line1_y))
        line.thickness = 5
        if k % 2 == 0:
        ...
        window.render()
        start_x = start_x + x - rectangle1.get_center().x
        start_y = start_y + y - rectangle1.get_center().y
